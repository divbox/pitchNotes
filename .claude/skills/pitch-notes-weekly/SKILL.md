...
- Before running: check `content.html`'s build date (the `data-build-date` on the edition block) and prose against what week it actually is. If it looks like it's still last week's content, say so and suggest running `pitch-notes-content` first rather than publishing stale news — this script has no way to detect that itself.
